...
extension DatabaseManager {
        ...
        ref.observeSingleEvent(of: .value) { [weak self] snapshot in
            ...
            strongSelf.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value) { snapshot in
                if var conversations = snapshot.value as? [[String: Any]] {
                    ...
                    //create
                    strongSelf.database.child("\(otherUserEmail)/conversations").setValue([recepient_newConversationData])
                }
            }
        ...
        database.child("\(id)/messages").observe(.value) { (snapshot) in
            ...
            let messages: [Message] = value.compactMap({ dictionary in
                ...
                if type == "photo" {
                    ...
                    print("rendering locaiton long=\(longitude), lat= \(latitude)")
    ...
    public func sendMessage(to conversation: String, otherUserEmail: String, name: String, newMessage: Message, completion: @escaping (Bool) -> Void) {
        ...
        // update recitient latest message
                    ...
                    strongSelf.database.child("\(currentEmail)/conversations").setValue(databaseEntryConversations) { error, _ in
                        ...
                        strongSelf.database.child("\(otherUserEmail)/conversations").observeSingleEvent(of: .value) { snapshot in
                            ...
                            if var otherUserConversations = snapshot.value as? [[String: Any]] {
                                ...
                                //current collection is not exist
                                let newConversationData: [String: Any] = [
                                    "id": conversation,
                                    "other_user_email": DatabaseManager.safeEmail(emailAddress: otherUserEmail),
                                    "name": currentName,
